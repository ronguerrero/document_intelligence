...
# DBTITLE 1,Imports, configuration, and helper functions
import io
import os
...
# DBTITLE 1,Document 1: TechNova Q1 2026 Earnings Report
...
# DBTITLE 1,Document 2: Meridian Infrastructure Fund III Memo
...
# DBTITLE 1,Document 3: Colombia Sovereign Credit Analysis
...
# DBTITLE 1,Document 4: NovaPharma LBO Deal Sheet
...
# DBTITLE 1,Document 5: GreenShield ESG Impact Fund Report
def gen_esg_report():
    buf = io.BytesIO()
    doc = SimpleDocTemplate(buf, pagesize=letter, leftMargin=.75*inch, rightMargin=.75*inch, topMargin=.75*inch, bottomMargin=.75*inch)
    s = []
...
# DBTITLE 1,Generate and save all PDFs to Volume
docs = [
...
for fname, label, gen_fn in docs:
    ...
    with open(path, "wb") as f:
        f.write(pdf)
    print(f"  OK: {label} ({len(pdf):,} bytes) -> {fname}")

print(f"\nDone! Files saved to {VOL}")